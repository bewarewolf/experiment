...
public class QuantilTestRunner extends BlockJUnit4ClassRunner {

	public static org.apache.logging.log4j.Logger LOG = LogManager.getRootLogger();
    ...
    //private final Logger logger = LoggerFactory.getLogger(QuantilTestRunner.class);
     ...
     */
    public QuantilTestRunner(final Class<?> klass) throws InitializationError {
     ...
     * @throws InitializationError if some JUnit-related initialization problem occurred
     */
    public QuantilTestRunner(Class<?> klass, Module module) throws InitializationError {
        this(klass, Injectors.getInjector(module));
    }

    public QuantilTestRunner(final Class<?> klass,
    ...
    public QuantilTestRunner(final Class<?> klass, final BatchManager batchManager) throws InitializationError {
        this(klass,
    ...
    public QuantilTestRunner(final Class<?> klass,
        ...
        this.methodScanner = new MethodScanner(configuration.getEnvironmentVariables());
    ...
    private void notifyTestSuiteFinished() {
        try {
        	...
        	LOG.error("Test event bus error: " + listenerException.getMessage(), listenerException);
        }
    }
    ...
    private boolean shouldRetryTest() {
        return configuration.getEnvironmentVariables().getPropertyAsBoolean("junit.retry_tests", false);
    }
    ...
    protected void runChild(FrameworkMethod method, RunNotifier notifier) {
        ...
        System.out.println(ProjectData.selectedEnvironment);
        System.out.println(ProjectData.selectedAPIEndpoint);
        if (shouldSkipTest(method) || !environmentMatches(method)) {
            return;
        }
        ...
        for (int attemptCount = 0; attemptCount <= maxRetries; attemptCount++) {
            ...
            if (attemptCount > 0) {
            	LOG.warn(method.getName() + " failed, making attempt " + (attemptCount + 1) + ". Max retries: " + maxRetries);
    ...
    private void clearMetadataIfRequired() {
        if (!configuration.getEnvironmentVariables().getPropertyAsBoolean("thucydides.maintain.session", false)) {
    ...
    private boolean shouldSkipTest(FrameworkMethod method) {
        ...
        boolean tag = tagScanner.shouldRunMethod(getTestClass().getJavaClass(), method.getName());
        boolean meth = methodScanner.shouldRunMethod(getTestClass().getJavaClass(), method.getName());
    ...
    private boolean environmentMatches(FrameworkMethod method) {
    	
    	Environment env = method.getAnnotation(Environment.class);
    	List<EnvironmentType> list = Arrays.asList(env.value());    	
    	
    	return env != null && 
    			(list.contains(ProjectData.selectedEnvironment) ||
    					list.contains(EnvironmentType.ANY));
    }
